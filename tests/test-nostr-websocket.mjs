...
import { WebSocket } from 'nostr-tools/relay';
...
async function testWebSocketConnection() {
  return new Promise((resolve) => {
    ...
    // Create WebSocket manually using browser/Node WebSocket API
    let ws;
    try {
        ...
        try {
          ...
          if (message[0] === 'EVENT' && message[1] === subscriptionId) {
            results.eventHandling = {
              status: 'pass',
              message: 'EVENT message received and parsed',
              time: Date.now() - reqStartTime
            };
          }
          ...
          if (message[0] === 'NOTICE') {
            ...
            if (message[1].toLowerCase().includes('whitelist') ||
              ...
              results.nip01Compliance.status = 'warning';
              results.nip01Compliance.message += ' (Whitelist authentication detected)';
            }
          }
    ...
    ws.addEventListener('close', () => {
      ...
      if (results.nip01Compliance.status === 'pending') {
        results.nip01Compliance = {
          status: 'warning',
          message: 'No EOSE received (possible whitelist restriction)',
          time: 0
        };
      }
      if (results.reqHandling.status === 'pending') {
        results.reqHandling = {
          status: 'warning',
          message: 'REQ message sent but response unclear',
          time: 0
        };
      }
      if (results.eventHandling.status === 'pending') {
        results.eventHandling = {
          status: 'info',
          message: 'No events received (empty relay or whitelist)',
          time: 0
        };
      }
...
async function runTests() {
  ...
  if (allPassed && !hasWarnings) {
    ...
    console.log('⚠ TESTS PASSED WITH WARNINGS (Whitelist authentication may be active)');
    process.exit(0);
  } else {
