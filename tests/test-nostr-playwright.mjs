...
// Use Playwright to test WebSocket (since it's already installed)
...
async function runTests() {
  ...
  // Navigate to a blank page to use browser WebSocket
  await page.goto('about:blank');

  // Run WebSocket tests in browser context
  const results = await page.evaluate(async (relayUrl) => {
    ...
    return new Promise((resolve) => {
          ...
          try {
            ...
            if (message[0] === 'EVENT' && message[1] === subscriptionId) {
              results.eventHandling = {
                status: 'pass',
                message: `EVENT received: kind ${message[2].kind}`,
                time: Date.now() - reqStartTime
              };
            }
            ...
            if (message[0] === 'NOTICE') {
              ...
              if (message[1].toLowerCase().includes('whitelist') ||
                ...
                results.nip01Compliance = {
                  status: 'warning',
                  message: `Auth/whitelist: ${message[1]}`,
                  time: Date.now() - reqStartTime
                };
              }
            }

            // Close after responses
      ...
      ws.addEventListener('close', () => {
        ...
        // Set defaults for pending tests
        if (results.nip01Compliance.status === 'pending') {
          results.nip01Compliance = {
            status: 'warning',
            message: 'No EOSE received (whitelist or timeout)',
            time: 0
          };
        }
        if (results.reqHandling.status === 'pending') {
          results.reqHandling = {
            status: 'warning',
            message: 'REQ sent but response unclear',
            time: 0
          };
        }
        if (results.eventHandling.status === 'pending') {
          results.eventHandling = {
            status: 'info',
            message: 'No events received (empty/whitelist)',
            time: 0
          };
        }
  ...
  if (allPassed && !hasWarnings) {
    ...
    console.log('Note: Relay may have whitelist authentication or other restrictions');
    process.exit(0);
  } else {
