...
function generateKeypairFromSeed(seed: string): { privkey: string; pubkey: string } {
...
function signEvent(event: NostrEvent, privkey: string): NostrEvent {
  // Serialize for ID
...
async function publishEvents(events: NostrEvent[]): Promise<number> {
